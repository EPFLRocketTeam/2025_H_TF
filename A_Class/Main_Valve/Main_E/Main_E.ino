...
int previous_b_Homing_E = 0, previous_b_SingleStep_E = 0; // Use for detecting state changes
...
void loop()
{
    ...
    // Single step
    if (b_SingleStep_E == 1 and previous_b_SingleStep_E == 0)
    {
...
void moveToTarget()
{
    ...
    double desired_speed; // Default speed
...
void read_data(bool serial_on = true)
{
    if (client.available())
    {
        ...
        {
            previous_b_Homing_E = b_Homing_E;
            b_Homing_E      = doc["b_Homing_E"].as<int>();
            previous_b_SingleStep_E = b_SingleStep_E;
            b_SingleStep_E  = doc["b_SingleStep_E"].as<int>();
            w_Main_EV       = doc["w_Main_EV"].as<int>();
        }
        if(serial_on)
        {
            Serial.print("b_Homing_E: ");
            Serial.print(b_Homing_E);
            Serial.print(", b_SingleStep_E: ");
            Serial.print(b_SingleStep_E);
            Serial.print(", w_Main_EV: ");
            Serial.println(w_Main_EV);
        }
    }
    else
    {
